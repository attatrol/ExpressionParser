...
{
    /**
     * This values represent order of execution for all executive tokens. They have to be different
     * to produce stable results. Do not change them unless you are sure what you are doing.
     * If executive tokens are in the same parentheses, these define their order of execution.
     */
            ...
            implements Comparator<Quartet<Token, Integer, Integer, Integer>>
    {
        ...
        @Override
        public int
                compare(Quartet<Token, Integer, Integer, Integer> arg0, Quartet<Token, Integer, Integer, Integer> arg1)
        {
    ...
    /**
     * Produces Expression from a LexerOutput instance.
     * Last must be a verified one.
     * 
     * @param lexerOutput 
     * @return
     */
    public static Expression processParsing(LexerOutput lexerOutput)
    {
        ...
        //execute parsing
        List<Quartet<Token, Integer, Integer, Integer>> sortedActions = getActionsSortedByExecutivePriority(
                lexemes);
        for (Quartet<Token, Integer, Integer, Integer> executiveToken : sortedActions) {
            ...
            if (token instanceof Function) {
                ...
                endCoord= getLastCoordinateOfFunction(lexemes,
                        tokenCoord + 2);
            ...
            IntermediateValue iv = new IntermediateValue(
                    intermediateValueOffset + actions.size());
    ...
    private static int[]
            resolveFunctionArgumentCoordinates(List<Lexeme> lexemes, int index, int arity)
    {
    ...
    private static int
            resolveArgument(List<Lexeme> lexemes, int index, int arity, Map<Argument, Integer> argumentLookup, List<Double> constants)
    {
    ...
    private static void
            setIntermediateValue(List<Lexeme> lexemes, IntermediateValue iv, int startCoord, int endCoord)
    {
    ...
    {
        return lexeme instanceof IntermediateValue
                || lexeme.getToken() instanceof Argument
